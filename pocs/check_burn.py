...
def checkBurn(chain, contract_address, pair_contract):
    try:
        print("chain: ", chain, "   checkBurn scan :   ", contract_address)
        ...
        if chain == "eth":
            ...
            fn_selector = function_signature_to_4byte_selector('burn(address,uint256)')
            ms_data = encode_abi(["address", "uint256"],
                                 [Web3.toChecksumAddress(pair_contract), 1])
            payload = {
                "id": 1,
                "jsonrpc": "2.0",
            ...
            if len(asset_changes) > 0:
                for change in asset_changes:
                    if change['changeType'] == "TRANSFER" \
                        ...
                        print("%s链：  合约地址：%s   存在免费burn漏洞，pair合约地址：%s" % (chain, contract_address, pair_contract))
                        scanutil.ding_send_text("[chain_poc]" + chain + "：  token合约疑似存在burn方法未鉴权的安全问题：" + contract_address + " pair合约地址：   " + pair_contract)
                        return
...
if __name__ == "__main__":
    checkBurn("eth", "0x0F67A226c385500c68fFa8bb7Fbe0DB15fE65E24", "0x0F67A226c385500c68fFa8bb7Fbe0DB15fE65E24")
